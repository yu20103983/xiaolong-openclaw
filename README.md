...
- 🎧 **蓝牙支持**：自动检测蓝牙耳机，支持全双工/半双工自适应，HFP 全双工模式可边说边听
...
HFP_DUPLEX = False       # HFP全双工: 输入输出都走HFP（音质低但支持边说边听）
...
**Q: 蓝牙耳机能不能边说边听（全双工）？**
在 `src/config.py` 中设置 `HFP_DUPLEX = True`，输入输出都走 HFP 通道，实现真全双工。音质会降为电话级（8-16kHz 单声道），但可以在播报时同时录音、随时打断。不开启则默认使用 HFP录音 + A2DP播放（音质好但需交替切换）。

**Q: Agent 响应超时/卡住不动？**
系统内置三层超时保护：Bridge 首次响应60s、停滞2分钟、Python兼底3分钟，超时后自动恢复并提示重试。如果使用较慢的模型（如 Opus）且上下文较长，可能触发超时，可考虑换用更快的模型或重启 session。
